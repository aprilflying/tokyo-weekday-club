...
const MIN_CLASS = 3;                // a session is confirmed, and charged, once this many children have a card on file
const AUTO_CONFIRM = process.env.AUTO_CONFIRM !== '0';   // set AUTO_CONFIRM=0 in Vercel to go back to charging by hand
...
async function onCardSaved(session) {
  ...
  // 4. did any of these sessions just reach the minimum? then charge every draft for that session
  if (AUTO_CONFIRM) await step('auto-confirm', () => autoConfirm(weeks));
}

async function autoConfirm(weekCodes) {
  if (!process.env.SHEET_URL) return;
  const counts = await (await fetch(`${process.env.SHEET_URL}?counts=1&fresh=1&_=${Date.now()}`, { redirect: 'follow' })).json();
  const min = counts.min || MIN_CLASS;
  const ready = weekCodes.filter(code => (counts[code] || 0) >= min);
  if (ready.length === 0) return;

  // all draft invoices in the account (drafts are few: one per unconfirmed booking-week)
  const drafts = [];
  for await (const inv of stripe.invoices.list({ status: 'draft', limit: 100 })) {
    if (inv.metadata && ready.includes(inv.metadata.week)) drafts.push(inv);
    if (drafts.length > 300) break;
  }

  for (const inv of drafts) {
    try {
      await stripe.invoices.finalizeInvoice(inv.id, { auto_advance: true });
      await stripe.invoices.pay(inv.id);          // charges the saved card now; invoice.paid → sheet "paid" + email
    } catch (err) {
      // declined, needs 3D Secure, or already finalized by a parallel booking: leave it to Stripe's retries and emails
      console.error('auto-confirm', inv.id, inv.metadata && inv.metadata.week, err && err.message);
    }
  }
}

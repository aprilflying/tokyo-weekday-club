...
const MAX_WEEKS  = 12;
const MAX_KIDS   = 4;
const MAX_CLASS  = 6;    // children per session; the sheet's counts endpoint can override
...
async function handle(request) {
  ...
  // ---- capacity: refuse any session that would go over the class maximum ----
  if (process.env.SHEET_URL) {
    try {
      const c = await (await fetch(`${process.env.SHEET_URL}?counts=1&_=${Date.now()}`, { redirect: 'follow' })).json();
      const max = c.max || MAX_CLASS;
      const full = weekCodes.filter(code => (c[code] || 0) + kids > max);
      if (full.length) {
        return Response.json({ ok: false, code: 'full', full, error: 'That session is full.' }, { status: 409 });
      }
    } catch (err) { console.error('counts check failed, allowing booking', err); }
  }
